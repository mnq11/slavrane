        ...
        <ThemeProvider theme={theme}>
            <TableContainer component={Paper} className={classes.root}>
                <Table>
                    <TableHead>
                        <TableRow>
                            <TableCell> <TableSortLabel active={sortColumn === 'Source'} direction={sortDirection}
                                                        onClick={() => handleSort('Source')}>المصدر</TableSortLabel></TableCell>
                            <TableCell align="right"><TableSortLabel active={sortColumn === 'Amount'}
                                                                     direction={sortDirection}
                                                                     onClick={() => handleSort('Amount')}>المبلغ</TableSortLabel></TableCell>
                            <TableCell align="right"><TableSortLabel active={sortColumn === 'Date'}
                                                                     direction={sortDirection}
                                                                     onClick={() => handleSort('Date')}>التاريخ</TableSortLabel></TableCell>
                            <TableCell align="right"><TableSortLabel active={sortColumn === 'Recurring'}
                                                                     direction={sortDirection}
                                                                     onClick={() => handleSort('Recurring')}>متكرر</TableSortLabel></TableCell>
                            <TableCell align="right"><TableSortLabel active={sortColumn === 'Frequency'}
                                                                     direction={sortDirection}
                                                                     onClick={() => handleSort('Frequency')}>التكرار</TableSortLabel></TableCell>
                    ...
                    <TableBody>
                        {sortedIncomes.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((income) => (
                            <TableRow key={income.IncomeID}>
                                ...
                                <TableCell
                                    align="right">{income.Date ? new Date(income.Date).toISOString().split('T')[0] : ''}</TableCell>
                                ...
                                <TableCell align="right">
                                    ...
                                    <IconButton color={"secondary"}
                                                onClick={() => handleDeleteIncome(income.IncomeID ?? 0)}>
                                        <DeleteIcon/>
                                    </IconButton>
                                </TableCell>
                            </TableRow>
